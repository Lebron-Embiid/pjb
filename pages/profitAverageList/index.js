...
import {
  queryAgentCouponDealRecordAverageOutlineIsDay,
  queryAgentCouponDealRecordAverageOutlineIsMonth,
  querySellCouponDealRecordAverageOutlineIsDay,
  querySellCouponDealRecordAverageOutlineIsMonth,
...
Page({
  ...
  getData(today){
    if(this.data.type == 0 || this.data.type == 1 || this.data.type == 4){
      ...
      if(this.data.selectDayMonth == 0){
        ...
        queryAgentCouponDealRecordAverageOutlineIsDay(data).then((res)=>{
          if(res.code == 200){
            let x_data = [[],[]];	
            for(let i in res.data){
              ...
              //交易成功数量
              x_data[0].push(Math.ceil(res.data[i].sealSuccessNumber))
              //传播数量
              x_data[1].push(Math.ceil(res.data[i].spreadNumber))
            }
            for(let i=0;i<2;i++){
        ...
        queryAgentCouponDealRecordAverageOutlineIsMonth(data).then((res)=>{
          if(res.code == 200){
            let x_data = [[],[]];	
            for(let i in res.data){
              if(res.data[i].sealSuccessNumber == null){
      ...
      if(this.data.selectDayMonth == 0){
        ...
        querySellCouponDealRecordAverageOutlineIsDay(data).then((res)=>{
          if(res.code == 200){
            let x_data = [];
            for(let i in res.data){
              ...
              x_data.push(Math.ceil(res.data[i].sellerNumber))
        ...
        querySellCouponDealRecordAverageOutlineIsMonth(data).then((res)=>{
          if(res.code == 200){
            let x_data = [];
            for(let i in res.data){
              if(res.data[i].sellerNumber == null){
  ...
  getOption(){
    ...
    let legend_arr = [];
    if(this.data.type == 0 || this.data.type == 1 || this.data.type == 4){
      title_txt = '代理人交易记录平均值报表';
      legend_arr = ['交易成功数量', '传播数量'];
    }else{
      title_txt = '销售员交易记录平均值报表';
      legend_arr = ['销售数量'];
    }
    var option = {
      title: {
