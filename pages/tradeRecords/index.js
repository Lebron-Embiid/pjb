...
import {
  queryBeerDeliveryStatisticsOutlineIsDay,
  queryBeerDeliveryStatisticsOutlineIsMonth
} from '../../api/user.js'
...
Page({
  ...
  backData(res){
    ...
    for(let i in res.data){
      ...
      //啤酒数量	
      x_data[0].push(res.data[i].deliveryBeerNumber)
      //配送数量	
      x_data[1].push(res.data[i].deliveryNumber)
    }
    for(let i=0;i<2;i++){
  ...
  getData(index){
    if(index == 0){
      ...
      queryBeerDeliveryStatisticsOutlineIsDay(data).then((res)=>{
        if(res.code == 200){
          this.backData(res)
        }
      })
      ...
      queryBeerDeliveryStatisticsOutlineIsMonth(data).then((res)=>{
        if(res.code == 200){
          this.backData(res)
        }
      })
